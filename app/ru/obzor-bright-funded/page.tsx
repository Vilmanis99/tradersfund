...
export default function RussianBrightFundedReviewPage() {
  return (
    <RussianPartnerReview
      ...
      lead={<>Bright Funded предлагает три разных набора правил: 1-Step, 2-Step Bright и 2-Step Classic. Все текущие ценовые уровни в захвате выражены в EUR, а add-on за больший сплит и временные скидки нельзя смешивать с базовой ценой.</>}
      countryNote={<>Русскоязычная страница не означает доступность для резидента России или другой страны. Перед оплатой подтвердите конкретные ограничения, KYC, валюту checkout и способ будущей выплаты на официальном сайте Bright Funded.</>}
      verdict={[
        { title: 'Bright Funded 1-Step', body: <>Один этап, цель 10%, дневной лимит 3% и трейлинг-максимальный убыток 6%. Базовый funded split — 80%; более высокий процент оформляется отдельно.</> },
      ...
      editorialNotes={[
        ...
        'Обычная первая выплата указана через 30 дней после первой funded-сделки, затем цикл 14 дней; в help-материалах есть конфликт о том, является ли bi-weekly add-on.',
        'Стандартный возврат не равен рекламному 100% refund add-on: эти условия разделены в захвате и не объединяются.',
      ]}
      faqs={[
        {
        ...
        },
        {
          ...
          a: 'Нет. В текущих данных базовый split — 80%; 90% указан как add-on ceiling. Сравнивайте фактическую цену checkout и условия выбранного add-on.',
        },
        {
          q: 'Можно ли зарегистрироваться из России?',
          a: 'Страница не подтверждает доступность. Проверьте актуальные country restrictions, KYC, оплату и выплаты на официальном checkout; обход ограничений VPN или неверными данными недопустим.',
        },
      ]}
    />
  )
