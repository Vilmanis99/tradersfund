...
const faqs: RussianFaqItem[] = [
  {
    ...
    a: 'По опубликованному описанию PropLive предоставляет торговые аккаунты для сделок на Московской бирже через брокера Финам и позиционирует себя как реальную проп-инфраструктуру. Это отличается от стандартной CFD-модели с автоматическим evaluation-челленджем, поэтому условия нельзя сравнивать только по проценту сплита.',
  },
  {
      ...
      <section className="ru-section">
        <div className="ru-shell ru-content">
          ...
          <div className="ru-grid">
            <article className="ru-card"><Scale size={22} color="var(--accent-light)" aria-hidden="true" /><h3>Рынок</h3><p className="ru-muted">PropLive описывает торговлю на Московской бирже через брокера; глобальные страницы сайта сравнивают отдельные CFD, крипто и instant-продукты по их собственным правилам.</p></article>
            ...
            <article className="ru-card"><WalletCards size={22} color="var(--accent-light)" aria-hidden="true" /><h3>Партнёрство</h3><p className="ru-muted">Публичная программа PropLive рассчитана на наставников и школы, поэтому мы не создаём локальный /go/ маршрут без подтверждённой редакционной affiliate-ссылки.</p></article>
          </div>
      ...
      <section className="ru-section">
        <div className="ru-shell" data-russian-local-global-funnel="proplive">
          <div className="ru-notice ru-disclosure">
            ...
            Если вам нужен сопоставимый онлайн-продукт, ниже находятся глобальные партнёры с отдельными свежими обзорами.
          ...
          <div className="ru-grid">
            {globalCards.map(item => (
              <article className="ru-card" key={item.slug} data-russian-local-global-partner={item.slug}>
                ...
                <p className="ru-muted">{item.products.length} свежих продуктов; проверьте страну, KYC, рынок, первую выплату и метод вывода до оплаты.</p>
                <div className="ru-actions">
